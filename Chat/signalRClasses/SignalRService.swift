...
//    }
    ...
    func connect() {
        ...
        // ✅ FIX: rebuild the connection so the URL always has the current
        // token and current active server — fixes stale/empty token baked
        // in at init time (fresh install / first login after launch).
        connection.stop()
